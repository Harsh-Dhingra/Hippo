-- Reverts 008_principal_identity.sql.
--
-- Both function bodies are duplicated here rather than referenced, because a
-- down migration that depends on reading another migration's file is a down
-- migration that breaks when that file moves.
--
-- Reverting narrows visibility: an account stops seeing what its owner's other
-- accounts were granted. That direction is the safe one, which is why this is
-- reversible at all.
...
AS $$
    ...
    query AS (
        SELECT plainto_tsquery('english', p_query_text) AS ts
...
-- The 003 closure: group membership only.
...
DROP INDEX IF EXISTS principals_identity_idx;
ALTER TABLE principals DROP COLUMN IF EXISTS identity_id;
