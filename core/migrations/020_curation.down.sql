-- Reverts 020_curation.sql by restoring the 014 ranking.
--
-- A ranking change only, in both directions: nothing becomes visible or
-- invisible, so reverting loses tuning rather than content. The signal and
-- supersede columns go with it, and the curation pass recomputes them.
...
DROP INDEX IF EXISTS chunks_superseded_idx;
DROP INDEX IF EXISTS chunks_uncurated_idx;
ALTER TABLE chunks DROP CONSTRAINT IF EXISTS chunks_cannot_supersede_itself;
ALTER TABLE chunks DROP CONSTRAINT IF EXISTS chunks_signal_is_a_weight;
ALTER TABLE chunks DROP COLUMN IF EXISTS curated_at;
ALTER TABLE chunks DROP COLUMN IF EXISTS superseded_by;
ALTER TABLE chunks DROP COLUMN IF EXISTS signal;
