-- Reverses 024_graph_walk.sql by restoring the 020 walk verbatim.
--
-- Which means restoring an undirected, untyped, unbounded recursion that
-- contributed nothing below k=20 and would not have survived a real corpus.
-- Kept faithful anyway: a down migration that improves on the thing it is
-- reverting to is not a reversal.
...
DROP FUNCTION IF EXISTS _edge_steps();
DROP FUNCTION IF EXISTS _edge_weight(text, boolean);
