...
- **P3-SRF-1 · Slack surface.** Ask/answer/approve from Slack itself. Meets users where they are; also the best demo distribution channel. Answers are ephemeral to the asker, never posted to the channel — a filtered answer put in #general is a leak dressed as a feature. Approving *is* allowed here, unlike from MCP, because Slack names the human who clicked.
...
- **P3-GRF-1 · The walk.** Typed, directional, bounded graph traversal. An edge is traversable *per direction*, and the closed directions are exactly the fan-out ones — person→their work, container→its contents. That bounds the walk by construction rather than by a tuned cap, and it is also what makes weighting meaningful. Done: recall at the default k improves and a seed inside a 3,000-message channel reaches zero irrelevant siblings.
- **P3-GRF-2 · The edges worth walking.** `blocks`, `resolved_by`, `references`, extracted deterministically from what sources already state — a Jira link type, "fixes #123" in a PR, a ticket key in a message. Weighted already by GRF-1. Done: "what is blocking X" is answered by a graph hop rather than by text search that happens to work.
